...
const sampleAlertmanagerResponse: {
  status: string;
...
describe('Flags', () => {
  ...
  describe('when data is returned', () => {
    it('renders a table', async () => {
      const mock = fetch.mockResponse(JSON.stringify(sampleAlertmanagerResponse));
      ...
      const keys = Object.keys(sampleAlertmanagerResponse.data);
      expect(rows.length).toBe(keys.length);
      for (let i = 0; i < keys.length; i++) {
        const row = rows.at(i);
        expect(row.find('th').text()).toBe(keys[i]);
        expect(row.find('td').text()).toBe(sampleAlertmanagerResponse.data[keys[i]]);
      }
    });
  });
